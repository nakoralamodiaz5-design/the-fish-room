...
function addCart(id){
  ...
  save();renderHeader();openCart();
}else state.cart.push({id,qty:1});save();renderHeader();openCart();}
 ...
 if(mode==="cart"){
  ...
  ${items.length ? items.map(x=>`
    <div class="drawer-item">
      ...
      <div>
        <b>${x.p.name}</b>
        <div>${x.p.price.toFixed(2)} € / ${x.p.unit}</div>
        <div style="display:flex;align-items:center;gap:7px;margin-top:8px">
          <button class="qty" onclick="changeQty('${x.p.id}',-1)">−</button>
          <strong>${x.qty}</strong>
          <button class="qty" onclick="changeQty('${x.p.id}',1)">+</button>
        </div>
      </div>
      <button onclick="removeCart('${x.p.id}')">✕</button>
    </div>`).join("")+
    `<div class="drawer-summary">
      <div><span>Productos</span><b>${cartItems().reduce((a,x)=>a+x.qty,0)}</b></div>
      <div><span>Recogida en tienda</span><b>Gratis</b></div>
      <div class="drawer-total"><span>Total</span><b>${cartTotal().toFixed(2)} €</b></div>
    </div>
    <button class="btn" style="margin-top:15px;width:100%" onclick="checkout()">Continuar con el pedido</button>
    <p style="color:#9cb4bc;font-size:11px">🏪 Recogida en The Fish Room · Santa María de Guía</p>`
...
function checkout(){
  ...
  if(!phone)return;
  const text=cartItems().map(x=>`${x.qty}x ${x.p.name} (${x.p.price.toFixed(2)} €)`).join("
");
  const msg=`Nuevo pedido The Fish Room%0A%0ACliente: ${name}%0ATeléfono: ${phone}%0A%0A${encodeURIComponent(text)}%0A%0ATotal: ${cartTotal().toFixed(2)} €%0ARecogida: tienda, Santa María de Guía`;
  location.href=`https://wa.me/34612412469?text=${msg}`;
}x ${x.p.name}`).join(", ");location.href=`https://wa.me/34612412469?text=${encodeURIComponent("Hola The Fish Room, quiero pedir: "+text+" . Total orientativo: "+cartTotal().toFixed(2)+" €. Quiero recogerlo en tienda.")}`}
