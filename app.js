...
function stockLabel(product) {
  if (product.status === "agotado") return ["Agotado", "off"];
...
function openCart() {
  ...
  content.innerHTML = `
    ...
    <p style="color:#9cb4bc;font-size:11px">
      🏪 Recogida en The Fish Room · Santa María de Guía
    </p>
  `;
}
  ...
  content.innerHTML = `
    ...
    ${items.length ? items.map((product) => `
      <div class="drawer-item">
        ...
        <button onclick="toggleFav('${product.id}');openFavs()">✕</button>
      </div>
...
function renderFilters() {
  ...
  filters.innerHTML = categories.map((category) => `
    <button class="chip ${category === activeCat ? "active" : ""}"
...
function setCategory(category) {
  ...
  const shop = $("#peces");
  if (shop) {
    window.scrollTo({ top: shop.offsetTop - 70, behavior: "smooth" });
  }
}
...
function renderProducts() {
  ...
  const sort = sortNode?.value || "featured";
        ...
        return `
          <article class="product">
            ...
            <div class="pbody">
              ...
              <div class="price-row">
                <span class="price">${product.price.toFixed(2).replace(".", ",")} € / ${product.unit}</span>
                <span class="stock ${stockClass}">${stockText}</span>
              </div>
              <div class="pactions">
                <button class="buy" onclick="addCart('${product.id}')">🛒 Añadir</button>
                <button class="fav heart ${favourite ? "on" : ""}"
                        onclick="toggleFav('${product.id}')">
                  ${favourite ? "♥" : "♡"}
                </button>
              </div>
              <a class="wh"
                 href="https://wa.me/34612412469?text=${encodeURIComponent(`Hola The Fish Room, me interesa ${product.name}. ¿Sigue disponible?`)}"
                 target="_blank">
...
function bookingSubmit(event) {
  ...
  alert("Solicitud guardada. En la versión publicada la conectaremos al sistema real de correo y WhatsApp.");
  event.target.reset();
}
  ...
  reviewsNode.innerHTML = reviews.length
    ...
    : `
      <article class="review">
        <div class="stars">★★★★★</div>
        <b>Próximamente</b>
        <p>Las primeras reseñas reales de The Fish Room aparecerán aquí.</p>
      </article>
    `;
}
